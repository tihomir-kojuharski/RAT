...
class ReplayBuffer:
    def __init__(self, start_index, end_index, batch_size, is_permed, coin_number, sample_bias=1.0):
        ...
        logging.info("buffer_bias is %f" % sample_bias)

        self.next = 0

    def append_experience(self, state_index):
        self.__experiences.append(Experience(state_index))
        logging.info("a new experience, indexed by %d, was appended" % state_index)
    ...
    def next_experience_batch(self):
        ...
        if self.__is_permed:
            ...
            # batch = self.__experiences[self.next:self.next + self.__batch_size]
            # self.next = (self.next + self.__batch_size) % (len(self.__experiences) - self.__batch_size)
        return batch
